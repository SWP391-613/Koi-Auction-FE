...
import { faCakeCandles, faFish, faRuler, faTicketSimple, faUser, faVenusMars } from "@fortawesome/free-solid-svg-icons";
  ...
  useEffect(() => {
    const fetchAuction = async () => {
      ...
      if (auctionData) {
        ...
        const koiDetailsPromises = auctionKoiData.map((auctionKoi: AuctionKoi) =>
  ...
  return (
    <>
      <div className="mx-auto flex flex-col">
        <div className="m-2 flex max-h-30 flex-col items-start justify-between gap-4
          ...
          {combinedKoiData.map((koi) => (
            ...
            >
              <div className="relative flex items-center justify-center bg-[#4086c7]">
                ...
                <div className="absolute top-2 left-2 bg-black bg-opacity-50
              ...
              <div className="p-4 bg-gray-200 rounded-b-lg">
                <h2 className="text-xl text-black font-semibold mb-2">{koi.name}</h2>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm text-gray-600">{koi.status_name}</span>
                  <span className={`text-xl font-bold px-2 py-1 rounded-full ${koi.auctionKoiData.current_bid
                    ? 'bg-green-500 text-white'
                    : 'bg-gray-500 text-white'
                    }`}>
                    ${koi.auctionKoiData.current_bid || koi.auctionKoiData.base_price}
                  </span>
                </div>
                <hr className="border-t border-gray-400 my-2" />
                <div className="grid grid-cols-2 lg:grid-cols-2 gap-2 text-sm">
                  <div className="flex items-center">
                    <FontAwesomeIcon icon={faFish} className="mr-2 text-gray-500 hidden sm:block" />
                    <label className="text-gray-500 text-xl ">Category: </label>
                    <span className="text-gray-500 text-xl"> {koi.category_id || 'Unknown'}</span>
                  </div>
                  <div className="flex items-center lg:justify-end">
                    <FontAwesomeIcon icon={faVenusMars} className="mr-2 text-gray-500 hidden sm:block" />
                    <label className="text-gray-500 text-xl ">Sex: </label>
                    <span className="text-gray-500 text-xl"> {koi.sex || 'Unknown'}</span>
                  </div>
                  <div className="flex items-center">
                    <FontAwesomeIcon icon={faRuler} className="mr-2 text-gray-500 hidden sm:block" />
                    <label className="text-gray-500 text-xl ">Length: </label>
                    <span className="text-gray-500 text-xl"> {koi.length}cm</span>
                  </div>
                  <div className="flex items-center lg:justify-end">
                    <FontAwesomeIcon icon={faCakeCandles} className="mr-2 text-gray-500 hidden sm:block" />
                    <label className="text-gray-500 text-xl ">Age: </label>
                    <span className="text-gray-500 text-xl"> {koi.age || 'Unknown'}</span>
                  </div>
                </div>
              </div>
            </Link>
